...
class NTfm3DOptimizer:
    ...
    def compute_loss(self, tfmparams, loss, vv, pts, masks, targets):
        ...
        # Compute loss
        tfms = torch.Tensor(tfmparams).view(bsz,nmaskch,3,4).type_as(pts).clone() # 3 x 4 matrix of params
        ...
        l = torch.pow(residual, 2).sum(1).view(-1).cpu().numpy() # "BHW" vector of losses
        for k in range(len(loss)):
            loss[k] = l[k]
        #return loss

    def compute_jac(self, tfmparams, loss, jac, vv, pts, masks, targets):
        # Setup loss computations
        bsz, nch, ht, wd = pts.size()
        nmaskch = masks.size(1)
        assert targets.is_same_size(pts), "Input/Output pts need to be of same size"
        ...
        # Compute loss
        tfms = torch.Tensor(tfmparams).view(bsz, nmaskch, 3, 4).type_as(pts)  # 3 x 4 matrix of params
        predpts = data.NTfm3D(pts, masks, tfms)  # Transform points through non-rigid transform
        ...
        v = self.jac.view(bsz,bsz,nmaskch,ht,wd,3,4).permute(0,3,4,1,2,5,6).clone().view(bsz*ht*wd, bsz*nmaskch*3*4).cpu().numpy()
...
def minimize(args):
    ...
    l = NTfm3DOptimizer()
    loss = lambda params, loss, v: l.compute_loss(params, loss, v, pts, masks, tgtpts)
    lossjac = lambda params, loss, jac, v: l.compute_jac(params, loss, jac, v, pts, masks, tgtpts)
    ...
    state = xalglib.minlmcreatevj(bsz*ht*wd, list(initparams))
