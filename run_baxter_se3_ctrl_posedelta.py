...
def setup_model(pose_checkpoint, trans_checkpoint, args, use_cuda=False):
    # # Create a model
    # if args.seq_len == 1:
    ...
    posemodel = ctrlnets.MultiStepSE3NoTransModel(num_ctrl=args.num_ctrl, num_se3=args.num_se3, delta_pivot=args.delta_pivot,
    ...
    posemaskpredfn = posemodel.forward_pose_mask
    if use_cuda:
    ...
    return posemodel, transmodel, posemaskpredfn
...
class SE3PoseSpaceOptimizer(object):
    def __init__(self, pargs):
        ...
        # Setup model
        self.posemodel, self.transmodel, self.posemaskfn = setup_model(self.pose_checkpoint, self.trans_checkpoint, self.args, self.pargs.cuda)
    ...
    def optimize_ctrl(self, poses, ctrl, goal_poses, pivots=None):
        ...
        if self.pargs.optimization == 'backprop':
            ...
            # FWD pass
            _, pred_poses_p = self.transmodel(inp)
            ...
            if self.pargs.gn_jac_check:
                ...
                poses_1 = util.to_var(poses.data, requires_grad=False)
                ctrl_1 = util.to_var(ctrl.data, requires_grad=True)
                _, pred_poses_1 = self.transmodel([poses_1, ctrl_1])
