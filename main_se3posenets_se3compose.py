...
def iterate(data_loader, model, tblogger, num_iters,
    ...
    se3tortlayer, tfmlayer = se3nn.SE3ToRt(args.se3_type), se3nn.NTfm3D()
    for i in xrange(num_iters):
        ...
        deltart01 = se3tortlayer(deltapose01)
        predpts = tfmlayer(pts[:,0], mask0, deltart01)
        ...
        if mask_consis_wt > 0:
            if args.pre_mask_consis:
                if args.mask_consis_loss_type == 'kl':
                    mask0in, mask1in = F.softmax(mask0pre), F.softmax(
                        mask1pre)  # Softmax the pre-sharpening activations
        ...
        with torch.no_grad():
            ...
            if i % args.disp_freq == 0:
                ...
                if i % args.imgdisp_freq == 0:
                    ...
                    depths = []
                    poses, transposes = [se3tortlayer(pose0),
                                         se3tortlayer(pose1)], \
                                        [se3tortlayer(transpose1)]
