...
def main():
    ...
    # Graph config
    print("Type of graph config: {}".format(args.graph_type))
...
def iterate(data_loader, model, tblogger, num_iters,
    ...
    for i in xrange(num_iters):
        ...
        for k in xrange(args.seq_len):
            ...
            nextpts = ptpredlayer()(currpts, initmask, deltap) # We do want to backpropagate to all the deltas in this case

            # Predict transformed normals and normalize them to get "unit" vectors
            if (args.normal_wt > 0):
                deltarot        = deltap.clone(); deltarot[:,:,:,3] = 0 # No translation
