...
public:
    ...
    inline void eliminatePlane(const float3 planeNormal, const float planeD, const float epsDist, const float epsNorm){
        dart::eliminatePlane(_vertMap->devicePtr(), _normMap->devicePtr(), _depthSource->getDepthWidth(), _depthSource->getDepthHeight(), planeNormal, planeD, epsDist, epsNorm);
        _vertsOnHost = false;
    }
