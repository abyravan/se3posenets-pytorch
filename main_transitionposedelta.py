...
def main():
    # Parse args
    global args, num_train_iter, num_train_pm_iter
    ...
    if args.evaluate:
        ...
        test_stats = iterate(test_loader, model, posemodel, tblogger, len(test_loader), mode='test')
    ...
    statstfile.write("Epoch, Loss, Consiserr\n")
    statsvfile.write("Epoch, Loss, Consiserr\n")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Train for one epoch
        train_stats = iterate(train_loader, model, posemodel, tblogger, args.train_ipe,
                           mode='train', optimizer=optimizer, epoch=epoch+1)
        train_ids += train_stats.data_ids

        # Evaluate on validation set
        val_stats = iterate(val_loader, model, posemodel, tblogger, args.val_ipe,
        ...
        statstfile.write("{}, {}, {}\n".format(epoch+1, train_stats.loss.avg, train_stats.consiserr.avg.sum()))
        statsvfile.write("{}, {}, {}\n".format(epoch + 1, val_stats.loss.avg, val_stats.consiserr.avg.sum()))
        ...
        save_checkpoint({
            ...
            'train_pm_iter' : num_train_pm_iter,
    ...
    test_loader = DataEnumerator(util.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False,
                                    ...
                                    collate_fn=test_dataset.collate_batch))
    test_stats = iterate(test_loader, model, posemodel, tblogger, len(test_loader),
...
def iterate(data_loader, model, posemodel, tblogger, num_iters,
            mode='test', optimizer=None, epoch=0):
    # Get global stuff?
    global num_train_iter, num_train_pm_iter
    ...
    for i in xrange(num_iters):
        ...
        pts      = util.to_var(sample['points'].type(deftype), requires_grad=False, volatile=not train)
        ...
        for k in xrange(pts.size(1)):
            if ((args.delta_pivot == '') or (args.delta_pivot == 'pred')) and args.pose_center == 'pred':
                ...
                if(k == 0):
                    p, initmask = posemodel.forward_pose_mask([netinput[:,k], jtangles[:,k]], train_iter=num_train_pm_iter)
                else:
                    p = posemodel.forward_only_pose([netinput[:,k], jtangles[:,k]])
                poses.append(p)
            else:
                p, m = posemodel.forward_pose_mask([netinput[:,k], jtangles[:,k]], train_iter=num_train_pm_iter) # TODO: Mask computations only needed for "maskmean" & "maskmeannograd"
                masks.append(m)
        ...
        for k in xrange(args.seq_len):
            ...
            if (k == 0):
                ...
                pose = util.to_var(transposes[k-1].data.clone(), requires_grad=True) # Use previous predicted pose (NOTE: This is a copy with the graph cut)

            # Predict next pose based on curr pose, control
            delta, trans = model([pose, ctrls[:,k]])
        ...
        loss, consisloss = 0, torch.zeros(args.seq_len)
        for k in xrange(args.seq_len):
            ...
            if args.consis_rt_loss:
                currconsisloss = args.loss_scale * ctrlnets.PoseC(transposes[k], poses[k+1])
            else:
                currconsisloss = args.loss_scale * ctrlnets.BiMSELoss(transposes[k], poses[k+1])

            # Append to total loss
            loss += currconsisloss # + currconsisloss # + currdissimposeloss + currdissimdeltaloss
        ...
        if i % args.disp_freq == 0:
            ...
            if args.use_wt_sharpening and not args.use_gt_masks:
                try:
                    noise_std, pow = posemodel.posemaskmodel.compute_wt_sharpening_stats(train_iter=num_train_pm_iter)
                except:
                    noise_std, pow = posemodel.maskmodel.compute_wt_sharpening_stats(train_iter=num_train_pm_iter)
                print('\tWeight sharpening => Num training iters: {}, Noise std: {:.4f}, Power: {:.3f}'.format(
                    num_train_pm_iter, noise_std, pow))
