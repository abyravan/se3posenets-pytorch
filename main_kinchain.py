...
def iterate(data_loader, model, tblogger, num_iters,
    ...
    for i in xrange(num_iters):
        ...
        ## Assume:
        ## poses are in local coords
        ## we get updated poses from transition model as (localpose * localdelta)
        localtransposes, localdeltaposes = [], []
        for k in xrange(args.seq_len):
            ...
            if (k == 0):
                localpose = localposes[0] # Use initial pose (NOTE: We want to backprop to the pose model here)
            else:
                localpose = util.to_var(localtransposes[k-1].data.clone(), requires_grad=False) # Use previous predicted pose (NOTE: This is a copy with the graph cut)

            # Predict next pose based on curr pose, control
            delta, trans = model.forward_next_pose(localpose, ctrls[:,k], jtangles[:,k],
                                               pivots[k] if (len(pivots) > 0) else None)
            localdeltaposes.append(delta)
            localtransposes.append(trans)
        ...
        # Now compute the global poses, global transposes and global deltas
        poses, deltaposes, transposes = [], [], []
        for k in xrange(len(localposes)):
            campose = se3nn.ComposeRt()(localposes[k]) # Compose the kinematic chain for the poses
            poses.append(campose)
            if (k < args.seq_len):
                # Compose the kinematic chain for the poses from the transition model
                camtranspose = se3nn.ComposeRt()(localtransposes[k])
                transposes.append(camtranspose)
        ...
        for k in xrange(args.seq_len):
            ...
            ### MEASURE THIS ERROR IN LOCAL POSE SPACE!
            ...
            localdelta = util.to_var(localdeltaposes[k].data.clone(), requires_grad=False)  # Break the graph here
            nextlocalpose_trans = se3nn.ComposeRtPair()(localposes[k], localdelta) # post-multiply
            if args.consis_rt_loss:
                currconsisloss = consis_wt * ctrlnets.PoseC(nextlocalpose_trans, localposes[k+1])
            else:
                currconsisloss = consis_wt * ctrlnets.BiMSELoss(nextlocalpose_trans, localposes[k+1])
        ...
        ### Measured in local pose space
        # Compute consistency error for display
        consiserror, consiserrormax = torch.zeros(args.seq_len), torch.zeros(args.seq_len)
        for k in xrange(args.seq_len):
            consiserrormax[k] = (localposes[k+1].data - localtransposes[k].data).abs().max()
            consiserror[k] = ctrlnets.BiAbsLoss(localposes[k+1].data, localtransposes[k].data)
