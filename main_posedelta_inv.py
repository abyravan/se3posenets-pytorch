...
# Supervised segmentation loss
parser.add_argument('--inverse-wt', default=0.0, type=float,
...
def main():
    ...
    if args.resume:
        ...
        if os.path.isfile(args.resume):
            ...
            best_filoss  = checkpoint['best_flowinv_loss'] if 'best_flowinv_loss' in checkpoint else float("inf")
            ...
            best_fiepoch = checkpoint['best_flowinv_epoch'] if 'best_flowinv_epoch' in checkpoint else 0
            print('==== Best validation loss: {} was from epoch: {} ===='.format(best_loss, best_epoch))
        else:
            print("=> no checkpoint found at '{}'".format(args.resume))
    else:
        best_loss, best_floss, best_filoss    = float("inf"), float("inf"), float("inf")
        best_epoch, best_fepoch, best_fiepoch = 0, 0, 0
    ...
    statstfile.write("Epoch, Loss, Ptloss, Invloss, Normalloss, Anchorloss, Flowerrsum, Flowerravg, Inverr, Inverrmax\n")
    statsvfile.write("Epoch, Loss, Ptloss, Invloss, Normalloss, Anchorloss, Flowerrsum, Flowerravg, Inverr, Inverrmax\n")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Find best losses
        val_loss, val_floss, val_filoss = val_stats.loss.avg, \
                                          val_stats.ptloss.avg.sum(), \
                                          val_stats.ptloss.avg.sum() + val_stats.invloss.avg.sum()
        is_best, is_fbest, is_fibest    = (val_loss < best_loss), (val_floss < best_floss), (val_filoss < best_filoss)
        prev_best_loss, prev_best_floss, prev_best_filoss    = best_loss, best_floss, best_filoss
        prev_best_epoch, prev_best_fepoch, prev_best_fiepoch = best_epoch, best_fepoch, best_fiepoch
        s, sf, sfi = 'SAME', 'SAME', 'SAME'
        if is_best:
            best_loss, best_epoch, s       = val_loss, epoch+1, 'IMPROVED'
        if is_fbest:
            best_floss, best_fepoch, sf    = val_floss, epoch+1, 'IMPROVED'
        if is_fibest:
            best_filoss, best_fiepoch, sfi = val_filoss, epoch+1, 'IMPROVED'
        ...
        print('==== [FILOSS] Epoch: {}, Status: {}, Previous best: {:.5f}/{}. Current: {:.5f}/{} ===='.format(
                                    epoch+1, sfi, prev_best_filoss, prev_best_fiepoch, best_loss, best_fiepoch))

        # Write losses to stats file
        statstfile.write("{}, {}, {}, {}, {}, {}, {}, {}, {}, {}\n".format(epoch+1, train_stats.loss.avg,
                                                                       train_stats.ptloss.avg.sum(),
                                                                       train_stats.invloss.avg.sum(),
                                                                       ...
                                                                       train_stats.inverr.avg.sum(),
                                                                       train_stats.inverrmax.avg.sum()))
        statsvfile.write("{}, {}, {}, {}, {}, {}, {}, {}, {}, {}\n".format(epoch + 1, val_stats.loss.avg,
                                                                       val_stats.ptloss.avg.sum(),
                                                                       val_stats.invloss.avg.sum(),
                                                                       ...
                                                                       val_stats.inverr.avg.sum(),
                                                                       val_stats.inverrmax.avg.sum()))
        ...
        save_checkpoint({
            ...
            'best_flowinv_loss'    : best_filoss,
            'best_epoch'           : best_epoch,
            'best_flow_epoch'      : best_fepoch,
            'best_flowinv_epoch'   : best_fiepoch,
    ...
    print('==== Best validation flow loss: {:.5f} was from epoch: {} ===='.format(checkpoint['best_flow_loss'],
                                                                         best_fepoch))
    print('==== Best validation flow-inv loss: {:.5f} was from epoch: {} ===='.format(checkpoint['best_flowinv_loss'],
                                                                         best_fiepoch))
    ...
    statsvfile.write("{}, {}, {}, {}, {}, {}, {}, {}, {}, {}\n".format(epoch+1, test_stats.loss.avg,
                                                                   ...
                                                                   test_stats.inverr.avg.sum(),
                                                                   test_stats.inverrmax.avg.sum()))
...
def iterate(data_loader, model, inv_model, tblogger, num_iters,
    ...
    stats.loss, stats.ptloss, stats.invloss     = AverageMeter(), AverageMeter(), AverageMeter()
    ...
    stats.inverr, stats.inverrmax               = AverageMeter(), AverageMeter()
    stats.data_ids = []
    if mode == 'test':
    ...
    pt_wt, inverse_wt, normal_wt = args.pt_wt * args.loss_scale, args.inverse_wt * args.loss_scale, args.normal_wt * args.loss_scale
    ...
    for i in xrange(num_iters):
        ...
        ### Compute controls based on the poses (t & t+1) using inverse model
        predctrls = []
        if (args.inverse_wt > 0):
        ...
        # We use point loss in the FWD dirn and Inverse loss between poses
        predpts, ptloss, invloss, loss = [], torch.zeros(args.seq_len), torch.zeros(args.seq_len), 0
        ...
        inverror, inverrormax = torch.zeros(args.seq_len), torch.zeros(args.seq_len)
        if (args.inverse_wt > 0):
            for k in xrange(args.seq_len):
                inverrormax[k] = (predctrls[k].data - ctrls[:,k].data).abs().max()
                inverror[k]    = ctrlnets.BiAbsLoss(predctrls[k].data, ctrls[:,k].data)
        stats.inverr.update(inverror)
        stats.inverrmax.update(inverrormax)
        ...
        if i % args.disp_freq == 0:
            ...
            info = {
                ...
                mode+'-invloss':  invloss.sum(),
                mode+'-normalloss': normalloss.sum(),
                mode+'-anchorloss': anchorloss.sum(),
                mode+'-inverr': inverror.sum(),
                mode+'-inverrmax': inverrormax.sum(),
...
def print_stats(mode, epoch, curr, total, samplecurr, sampletotal,
    ...
    print('Mode: {}, Epoch: [{:2}/{:2}], Iter: [{:4}/{:4}], Sample: [{:5}/{:5}], Batch size: {:2}, '
    ...
    for k in xrange(args.seq_len):
        print('\tStep: {}, Pt: {:6.3f} ({:6.3f}), Norm:  {:5.3f} ({:5.3f}), '
              'Inv: {:5.3f}/{:6.4f} ({:5.3f}/{:6.4f}) ({:5.3f}/{:6.4f}), ' 
              'Flow => Sum: {:6.3f} ({:6.3f}), Avg: {:5.3f} ({:5.3f}), '
              'Motion/Still => Sum: {:6.3f}/{:6.3f}, Avg: {:5.3f}/{:5.3f}'
            .format(
            1 + k * args.step_len,
            stats.ptloss.val[k], stats.ptloss.avg[k],
            stats.normalloss.val[k], stats.normalloss.avg[k],
            stats.invloss.val[k], stats.invloss.avg[k],
            stats.inverr.val[k], stats.inverr.avg[k],
            stats.inverrmax.val[k], stats.inverrmax.avg[k],
...
### Save checkpoint
def save_checkpoint(state, is_best, is_fbest=False, is_fibest=False, savedir='.', filename='checkpoint.pth.tar'):
    ...
    if is_fibest:
        shutil.copyfile(savefile, savedir + '/model_flowinv_best.pth.tar')
