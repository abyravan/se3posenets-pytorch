...
def main():
    ...
    # Loss parameters
    print('Loss scale: {}, Pose loss wt => {}'.format(args.loss_scale, args.loss_wt))
    ...
    ### Load the model
    num_train_iter = 0
    modelfn = se2nets.SE2PoseEncoder if args.se2_data else ctrlnets.PoseEncoder
    model = modelfn(num_se3=args.num_se3, se3_type=args.se3_type, use_pivot=args.pred_pivot,
                    use_kinchain=False, pre_conv=args.pre_conv,
                    input_channels=3, use_bn=args.batch_norm, nonlinearity=args.nonlin,
                    init_se3_iden=args.init_posese3_iden, wide=args.wide_model)
    if args.cuda:
...
def iterate(data_loader, model, tblogger, num_iters,
    ...
    lossm, poselossm_1, poselossm_2 = AverageMeter(), AverageMeter(), AverageMeter()
    ...
    for i in xrange(num_iters):
        ...
        # GT poses
        tarpose_1 = util.to_var(sample['poses'][:, 0].clone().type(deftype), requires_grad=False)  # 0 is BG, so we add 1
        tarpose_2 = util.to_var(sample['poses'][:, 1].clone().type(deftype), requires_grad=False)
        ...
        # FWD pass
        pose_1 = model(pts_1)
        pose_2 = model(pts_2) # Just predict twice

        ### Pose error
        pose_wt = args.loss_wt * args.loss_scale
        poseloss_1 = pose_wt * ctrlnets.BiMSELoss(pose_1, tarpose_1)
        poseloss_2 = pose_wt * ctrlnets.BiMSELoss(pose_2, tarpose_2)

        # Compute total loss as sum of all losses
        loss = poseloss_1 + poseloss_2
        ...
        if (i % args.disp_freq == 0) or (i == num_iters-1):
            ...
            print('\tPose-Loss-1: {pos1.val:.5f} ({pos1.avg:.5f}), '
                ...
                .format(pos1=poselossm_1, pos2=poselossm_2,
            ...
            if i % args.imgdisp_freq == 0:
                ...
                gtpose_1, gtpose_2 = tarpose_1[id].data.cpu(), tarpose_2[id].data.cpu()
                predpose_1, predpose_2 = pose_1[id].data.cpu(), pose_2[id].data.cpu()
                ...
                print '\tPredicted delta-SE3s @ t=2:', deltapose_t_12.data[id].view(args.num_se3, -1).cpu()
    ...
    return lossm, poselossm_1, poselossm_2, deltaroterrm, deltatranserrm
