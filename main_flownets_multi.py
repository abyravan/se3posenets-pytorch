...
def iterate(data_loader, model, tblogger, num_iters,
    ...
    for i in xrange(num_iters):
        ...
        with torch.no_grad():
            ...
            predflows_t = torch.cat([(x - pts[:,0]).unsqueeze(1) for x in predpts], 1)
            flows_t = fwdflows
            if args.use_only_da_for_flows:
                ...
                flowerr_sum, flowerr_avg, \
                    ...
                    still_err, still_npt         = helpers.compute_masked_flow_errors(predflows_t * fwdvis, flows_t) # Zero out flows for non-visible points
            else:
                flowerr_sum, flowerr_avg, \
                    motionerr_sum, motionerr_avg, \
                    stillerr_sum, stillerr_avg, \
