...
void run_pangolin(const boost::shared_ptr<PyRealData> data)
{
    ...
    //cudaGLSetGLDevice(0);
    //cudaDeviceReset();
    const float totalwidth = 320*4 + panelWidth;
...
//                .SetHandler(new pangolin::Handler3D(camState));
    ...
    if (data->use_simulator) camDisp.SetHandler(new pangolin::Handler3D(camState));
...
//    Eigen::Matrix4f modelViewPoseInv = modelViewPose.inverse();
    ...
    pangolin::GlFont glFont("/home/barun/Projects/ExternalPackages/Pangolin/src/_embed_/fonts/AnonymousPro.ttf", 24.0);
    ...
    datac = boost::shared_ptr<PyRealData>(new PyRealData(data->imgHeight, data->imgWidth,
                                                         ...
                                                         data->fx, data->fy,
                                                         data->cx, data->cy, data->savedir, data->use_simulator));
                                   ...
                                   int save_frame)
{
    ...
    memcpy(data->curr_rgb, curr_rgb, data->imgHeight * data->imgWidth * 3 * sizeof(unsigned char));
                                   ...
                                   const float *start_deg_errors)
{
    ...
    memcpy(data->init_rgb,  start_rgb, data->imgHeight * data->imgWidth * 3 * sizeof(unsigned char));
    memcpy(data->final_rgb, goal_rgb,  data->imgHeight * data->imgWidth * 3 * sizeof(unsigned char));
...
///
void RealCtrlViz::compute_gt_da(const float *input_jts,
...
void RealCtrlViz::initialize_problem(const float *start_jts, const float *goal_jts,
