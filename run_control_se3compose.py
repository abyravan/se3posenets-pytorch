...
def main():
    ...
    for k in xrange(num_configs):
        ...
        init_pose_err_indiv = args.loss_scale * (start_poses - goal_poses).pow(2).view(args.num_se3, -1).mean(1)
        ...
        start_poses_f, goal_poses_f = se3nn.SE3ToRt(args.se3_type)(start_poses).data.cpu().float(), \
                                      se3nn.SE3ToRt(args.se3_type)(goal_poses).data.cpu().float()
        ...
        for it in xrange(pargs.max_iter):
            ...
            # Get CPU stuff
            curr_poses_f, curr_masks_f = se3nn.SE3ToRt(args.se3_type)(curr_poses).data.cpu().float(), \
                                         curr_masks.data.cpu().float()
            ...
            curr_pose_err_indiv = args.loss_scale * (curr_poses - goal_poses).pow(2).view(args.num_se3, -1).mean(1)
