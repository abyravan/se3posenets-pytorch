...
def main():
    ...
    if cargs.real_data:
        posemodelfile = "icra18results/initsubmission/networks/realdata/oursbestfixed/8se3_wtsharpenr1s0_1seq_normmsesqrt_motionnormloss_rlcheck/model_best.pth.tar"
    else:
        posemodelfile = "icra18results/initsubmission/networks/alldata/oursbestfixed/8se3_wtsharpenr1s0_1seq_normmsesqrt_motionnormloss/model_best.pth.tar"
    ...
    ## TODO: Load SE3-Net
    if cargs.real_data:
        se3modelfile = "icra18results/initsubmission/networks/realdata/se3/8se3_wtsharpenr1s0_1seq_normmsesqrt_motionnormloss_rlcheck/model_best.pth.tar"
    else:
        se3modelfile = "icra18results/initsubmission/networks/alldata/se3/8se3_wtsharpenr1s0_1seq_normmsesqrt_motionnormloss/model_best.pth.tar"
    ...
    ## TODO: Load SE3-Pose-Net
    if cargs.real_data:
        flowmodelfile = "icra18results/initsubmission/networks/realdata/flow/1seq_normmsesqrt_motionnormloss_rlcheck/model_best.pth.tar"
    else:
        flowmodelfile = "icra18results/initsubmission/networks/alldata/flow/1seq_normmsesqrt_motionnormloss/model_best.pth.tar"
    ...
    while len(ids) < cargs.num_examples:
        ...
        for k in xrange(cargs.seq_len):
            ...
            currpts = pts[:,0] if (k == 0) else predpts1['se3posepts'][k-1]
            ...
            # Back-project to 2D and project back to 3D
            intrin = args.cam_intrinsics[0]
            proj2d = se3nn.Dense3DPointsToRenderedSubPixelDepth(intrin['fy'], intrin['fx'], intrin['cy'], intrin['cx'])(nextpts)
            proj3d = se3nn.DepthImageToDense3DPoints(img_ht, img_wd, intrin['fy'], intrin['fx'], intrin['cy'], intrin['cx'])(proj2d.narrow(1,2,1).clone())
            predpts1['se3posepts'].append(proj3d)
        ...
        for k in xrange(cargs.seq_len):
            ...
            currpts = pts[:,0] if (k == 0) else predpts1['se3'][k-1]
            ...
            nextpts = currpts + flows
            predpts['se3'].append(nextpts)

            # Back-project to 2D and project back to 3D
            intrin = args.cam_intrinsics[0]
            proj2d = se3nn.Dense3DPointsToRenderedSubPixelDepth(intrin['fy'], intrin['fx'], intrin['cy'], intrin['cx'])(nextpts)
            proj3d = se3nn.DepthImageToDense3DPoints(img_ht, img_wd, intrin['fy'], intrin['fx'], intrin['cy'],
                                                     intrin['cx'])(proj2d.narrow(1, 2, 1).clone())
            predpts1['se3'].append(proj3d)
