...
def iterate(data_loader, model, tblogger, num_iters,
    ...
    for i in xrange(num_iters):
        ...
        ###### Run a FWD pass through the network (single-step)
        ### 1) Predict the poses and masks
        pose0 = util.to_var(sample['poses'][:,0].type(deftype).clone(), requires_grad=train)  # Use GT poses
        pose1 = util.to_var(sample['poses'][:,1].type(deftype).clone(), requires_grad=train)  # Use GT poses
        mask0 = util.to_var(sample['masks'][:,0].type(deftype).clone(), requires_grad=train)  # Use GT masks
        mask1 = util.to_var(sample['masks'][:,1].type(deftype).clone(), requires_grad=train)  # Use GT masks
        poses, masks, initmask = [pose0, pose1], [mask0, mask1], mask0

        ### 2) Predict the change in poses using the transition model
        inp = [pose0, jtangles[:,0], ctrls[:,0]] if args.use_jt_angles_trans else [pose0, ctrls[:,0]]
        delta, trans = model(inp)
        ...
        delta_g    = se3nn.ComposeRtPair()(pose1, se3nn.RtInverse()(pose0))  # pose1_g * pose0_g^-1 (GT delta pose)
...
def print_stats(mode, epoch, curr, total, samplecurr, sampletotal,
    ...
    for k in xrange(args.seq_len):
        print('\tStep: {}, Rot: {:.3f} ({:.3f}), Trans: {:.3f} ({:.3f}), '
              'Flow => Sum: {:.3f} ({:.3f}), Avg: {:.3f} ({:.3f}), '
              'Motion/Still => Sum: {:.3f}/{:.3f}, Avg: {:.3f}/{:.3f}'
            .format(
            1 + k * args.step_len,
            stats.rotloss.val[k], stats.rotloss.avg[k],
            stats.transloss.val[k], stats.transloss.avg[k],
