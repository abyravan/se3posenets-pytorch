...
def main():
    ...
    statstfile.write("Epoch, Loss, Ptloss, Consisloss, Maskconsisloss, Flowerrsum, Flowerravg, Consiserr\n")
    statsvfile.write("Epoch, Loss, Ptloss, Consisloss, Maskconsisloss, Flowerrsum, Flowerravg, Consiserr\n")
...
def iterate(data_loader, model, tblogger, num_iters,
    ...
    for i in xrange(num_iters):
        ...
        predposes, predmasks, predpremasks = [], [], []
        for k in xrange(pts.size(1)):
            # Predict the current pose and mask
            currpose, currmask = model.forward_pose_mask([netinput[:,k], jtangles[:,k]],
                                                         train_iter=num_train_iter)
            currpremask        = model.posemaskmodel.pre_sharpen_mask.clone()
        ...
        # Run through the transition model and predict the next set of points
        preddeltas, predtransposes, predpts = [], [], []
        for k in xrange(args.seq_len):
            # Make a prediction through the transition model
            # TODO: Do we cut the graph here??
            currpose = predposes[0] if (k == 0) else predtransposes[-1]
            deltapose, transpose = model.forward_next_pose(currpose, ctrls[:,k])
            ...
            if args.compose_deltas:
                currdeltapose = deltapose if (k == 0) else se3.ComposeRtPair(deltapose, preddeltas[-1]) # p_k * p_(k-1)^-1 * p_(k-1) * p_0^-1
            else:
                currdeltapose = se3.ComposeRtPair(transpose, se3.RtInverse(predposes[0])) # p_k * p_0^-1
            preddeltas.append(currdeltapose) # Save deltas
            predtransposes.append(transpose) # Save predicted next pose
        ...
        loss, ptloss, consisloss, maskconsisloss = 0, torch.zeros(args.seq_len), torch.zeros(args.seq_len), \
                                                   torch.zeros(args.seq_len)
        for k in xrange(args.seq_len):
            ### 3D loss
            ...
            # TODO: We can introduce some cuts to the graph to ensure that this loss goes to the poses only (maybe just the encoder poses?)
            # Poses from encoder @ t & @ t+1 should be separated by delta from t->t+1
            currconsisloss = consis_wt * ctrlnets.BiMSELoss(predtransposes[k], predposes[k+1])
        ...
        predflows = torch.cat([(predpts.data - pts.data[:,0]).unsqueeze(1)], 1)
        flows = fwdflows.data
        ...
        for k in xrange(args.seq_len):
            consiserrormax[k] = (predposes[k+1].data - predtransposes[k].data).abs().max()
            consiserror[k] = ctrlnets.BiAbsLoss(predposes[k+1].data, predtransposes[k].data)
        ...
        if i % args.disp_freq == 0:
            ...
            if i % args.imgdisp_freq == 0:
                ...
                for k in xrange(args.seq_len+1):
                    ...
                    predposet = predtransposes[k-1].data[id].cpu().float() if (k > 0) else None
                ...
                maskdisp  = torchvision.utils.make_grid(torch.cat([mask0.data.narrow(0,id,1)], 0).cpu().view(-1, 1, args.img_ht, args.img_wd),
