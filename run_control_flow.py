...
def main():
    ...
    if not hasattr(args, "use_se3_nets"):
        args.use_se3_nets = False
    ...
    for k in xrange(num_configs):
        ...
        for it in xrange(pargs.max_iter):
            ...
            # Need to DA current points to the initial points
            gtwarped_out = torch.zeros(1,3,240,320)
            gtda_ids     = torch.zeros(1,1,240,320)
            ...
            print('Gen: {:.3f}({:.3f}), Viz: {:.3f}({:.3f}),'
                  ' Optim: {:.3f}({:.3f}), Rest: {:.3f}({:.3f})'.format(
                gen_time.val, gen_time.avg,
...
def optimize_ctrl(model, pts, ctrl, angles, warpedtarget, warpedmask):
    ...
    if pargs.optimization == 'backprop':
        ...
        # FWD pass + loss
        pts_1  = util.to_var(pts, requires_grad=False)
        ...
        loss = err.dot(err) * (args.loss_scale / err.nelement())
        ...
        # Do perturbation
        pts_p    = util.to_var(pts.repeat(nperturb + 1, 1, 1, 1))  # Replicate poses
        angles_p = util.to_var(angles.repeat(nperturb + 1, 1).type_as(pts))  # Replicate poses
        ...
        # Compute Jacobian
        Jt = pred_flows_p.data[1:].view(nperturb, -1).clone()  # nperturb x posedim
        Jt -= pred_flows_p.data.narrow(0, 0, 1).view(1, -1).expand_as(Jt)  # [ f(x+eps) - f(x) ]
        ...
        ctrl_grad = torch.mm(Jinv, torch.mm(Jt, pred_flows.grad.data.view(-1, 1)))  # (J^t*J + \lambda I)^-1 * (Jt * g)
