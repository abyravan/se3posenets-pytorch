...
parser.add_argument('--coord-iter', default=0, type=int,
                    metavar='N', help='Number of gradient descent steps with each model before switching to the other '
                                      '(default: 0 = train both at each step, k > 0 train mask model for k steps, '
                                      'then train pose/trans model for k steps and so on )')
...
def main():
    ...
    ### Load the model
    num_train_iter = 0
    model = ctrlnets.SE3DecompModel(num_ctrl=args.num_ctrl, num_se3=args.num_se3,
    ...
    if (args.coord_iter > 0):
        args.train_ipe += 2*args.coord_iter - (args.train_ipe % (2*args.coord_iter)) # Add enough iterations so that we have a full final round of co-ordinate descent
        print('Updated training iters per epch to {} to allow for a full final round of co-ordinate training'.format(args.train_ipe))
...
def iterate(data_loader, model, tblogger, num_iters,
    ...
    for i in xrange(num_iters):
        ...
        iterprint = ''
        if train and args.coord_iter > 0:
            j = i % (2*args.coord_iter) # One round of training mask-net & pose/trans-net is = 2*coord_iter
        ...
        pts_1    = util.to_var(sample['points'][:, 0].clone().type(deftype), requires_grad=train) # Need grad only for training!
        pts_2    = util.to_var(sample['points'][:, 1].clone().type(deftype), requires_grad=train) # Need grad only for training!
        ctrls_1  = util.to_var(sample['controls'][:, 0].clone().type(deftype), requires_grad=train) # Need grad only for training!
...
def print_stats(mode, epoch, curr, total, samplecurr, sampletotal,
    ...
    print('Mode: {}, Epoch: [{}/{}], Iter: [{}/{}], Sample: [{}/{}], '
        ...
        sampletotal, loss=loss, fwd=fwdloss, bwd=bwdloss, consis=consisloss))
...
### Enable training for all layers of a model
def enable_training(model):
    model.train() # Set in training mode
    for param in model.parameters():
        param.requires_grad = True

### Disable training for all layers of a model
