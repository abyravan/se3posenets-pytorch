...
class ReorgFunction(Function):
    ...
    def forward(self, x):
        ...
        if x.is_cuda:
            out = out.cuda()
            se3layers.reorg_cuda(x, out_w, out_h, out_c, bsize, stride, 0, out)
        else:
            se3layers.reorg_cpu(x, out_w, out_h, out_c, bsize, stride, 0, out)
    ...
    def backward(self, grad_top):
        ...
        if grad_top.is_cuda:
            ...
            se3layers.reorg_cuda(grad_top, w, h, c, bsize, stride, 1, grad_bottom)
        else:
            se3layers.reorg_cpu(grad_top, w, h, c, bsize, stride, 1, grad_bottom)
