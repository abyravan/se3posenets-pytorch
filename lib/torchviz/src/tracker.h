...
public:
    ...
    void setFrameOnGPU(const int frame);
    ...
    const int * getDeviceIntersectionPotentialMatrix(const int modelNum) const { return _intersectionPotentialMatrices[modelNum]->devicePtr(); }
...
//protected:
    ...
    //void removePrior(int model,int frame);
    //void removePriors(){_priors.clear();}
