...
class RoIPoolFunction(Function):
    ...
    def forward(self, features, rois):
        ...
        if not features.is_cuda:
            ...
            se3layers.roi_pooling_forward(self.pooled_height, self.pooled_width, self.spatial_scale,
                                            _features, rois, output)
            # output = output.cuda()
        else:
            output = output.cuda()
            argmax = argmax.cuda()
            se3layers.roi_pooling_forward_cuda(self.pooled_height, self.pooled_width, self.spatial_scale,
    ...
    def backward(self, grad_output):
        ...
        se3layers.roi_pooling_backward_cuda(self.pooled_height, self.pooled_width, self.spatial_scale,
