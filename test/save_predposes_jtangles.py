...
def iterate(dataset, model, mode='test'):
    ...
    for i in xrange(1000):
        ...
        pts       = util.req_grad(sample['points'].to(device), False)  # Need gradients

        # Get XYZRGB input
        chdim = 2 if pargs.use_data_loader else 1 # It is B x S x C x H x W if we use data loader
        if args.use_xyzrgb:
            rgb = util.req_grad(sample['rgbs'].to(device) / 255.0, False)  # Normalize RGB to 0-1
            netinput = torch.cat([pts, rgb], chdim)  # Concat along channels dimension
        elif args.use_xyzhue:
            hue = util.req_grad(sample['rgbs'].narrow(2, 0, 1).to(device) / 179.0, False)  # Normalize Hue to 0-1 (Opencv has hue from 0-179)
            netinput = torch.cat([pts, hue], chdim)  # Concat along channels dimension
        else:
            netinput = pts  # XYZ

        # Get jt angles
        jtangles = util.req_grad(sample['actctrlconfigs'].to(device), False)
        ...
        with torch.no_grad():
            ...
            for kk in xrange(bsz):
                did = sample['datasetid'][kk].item() if pargs.use_data_loader else sample['datasetid'].item()  # Dataset ID
                fid = sample['folderid'][kk].item() if pargs.use_data_loader else sample['folderid'].item()   # Folder ID in dataset
                ...
                # Save to list
                if pargs.use_data_loader:
                    pred_poses_d[-1].append(pred_poses[kk:kk+1].cpu().clone())
                    ...
                    pred_poses_d[-1].append(pred_poses.cpu().clone())
