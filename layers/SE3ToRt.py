...
class SE3ToRtFunction(Function):
	...
	def forward(self, input):
		...
		if (self.transform_type == 'se3euler'): # parameters are [dx, dy, dz, theta1, theta2, theta3]
			...
			# R = Rz(t3) * Ry(t2) * Rx(t1)
			torch.bmm(rotzy, rotx, out = output.view(tot_se3,3,num_cols).narrow(2,0,3)) # R = Rzyx

		elif (self.transform_type == 'se3aa'):
			# Create rot from aa
			output.narrow(3,0,3).copy_(self.create_rot_from_aa(rot_params))
		elif (self.transform_type == 'se3quat'):
			# Compute the unit quaternion
			unitquat = self.create_unitquat_from_quat(rot_params)

			# Compute rotation matrix from unit quaternion
			output.narrow(3,0,3).copy_(self.create_rot_from_unitquat(unitquat))
		...
		output.narrow(3,3,1).copy_(params.narrow(1,0,3)) # [tx,ty,tz] (B x k x 3)

		####
		# Pivot vector (3x1)
		if self.has_pivot:
			output.narrow(3,4,1).copy_(params.narrow(1,3+rot_dim,3)) # [px, py, pz] (B x k x 3)
	...
	def backward(self, grad_output):
		...
		if (self.transform_type == 'se3euler'): # parameters are [dx, dy, dz, theta1, theta2, theta3]
			...
			for k in xrange(3):
				...
				vec		= torch.zeros(1,3).type_as(gradr); vec[0][k] = 1 # Unit vector
				...
				gradr.copy_((-Rv * grad_output_v.narrow(2,0,3)).sum(1).sum(2))
		elif (self.transform_type == 'se3aa'):
			...
			axis 	= params.view(tot_se3,-1,1).narrow(1,3,rot_dim) # (Bk) x 3 x 1 => 3 unnormalized AA parameters (note that first 3 parameters are [t])
			...
			vV	= (vV + vI) / (angle2.expand_as(vV))	# (Bk) x 3 x 3 => [v * v' + v x (Id - R)] / ||v||^2
			...
			# Compute the unit quaternion
			quat 	 = rot_params
			unitquat = self.create_unitquat_from_quat(quat)
			...
			grad_rot_params.copy_((dRdq * grad_out).sum(1).sum(2)) # (Bk) x 4
		elif (self.transform_type == 'se3spquat'):
			# Compute the unit quaternion
			spquat = rot_params
