...
bool ImageDepthSource<DepthType,ColorType>::initialize(const std::string depthFilenameFormat,
    ...
    else if (depthExtension == "jpg") {
        _depthImageType = IMAGE_JPG;
    }
    else {
    ...
    do {
        ...
        if (exists) _depthFilenames.push_back(filename); // Add to list of files
    ...
    if (hasColor) {
        ...
        else if (colorExtension == "jpg") {
            _colorImageType = IMAGE_JPG;
        }
        else {
        ...
        do {
            ...
            exists = (stat(filename.c_str(),&buffer) == 0);
            if (exists) _colorFilenames.push_back(filename);
