...
def iterate(data_loader, model, tblogger, num_iters,
    ...
    for i in xrange(num_iters):
        ...
        with torch.no_grad():
            ...
            for k in xrange(args.seq_len):
                consiserrormax[k] = (predposes[k+1] - predtransposes[k]).abs().max()
                consiserror[k] = ctrlnets.BiAbsLoss(predposes[k+1], predtransposes[k])
            ...
            if i % args.disp_freq == 0:
                ...
                if i % args.imgdisp_freq == 0:
                    ...
                    for k in xrange(args.seq_len+1):
                        ...
                        predposet = predtransposes[k-1][id].cpu().float() if (k > 0) else None
